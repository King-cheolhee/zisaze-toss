...
1. 상세→뒤로 시 홈 상태(검색·필터·스크롤) 초기화 2. API fetch 타임아웃/Abort 없음 3. 삭제 모달 접근성(포커스 트랩·ESC) 4. 활성 탭 재클릭 시 해시 히스토리 중복 5. 연속 제거 모두 실패 시 복원 순서 어긋날 수 있음(새로고침 시 서버 순서로 정착).

**알려진 한계(의도된 트레이드오프, 4차 리뷰에서 미수정 판정)** — 익명키 기록이 유실된 "소유자 불명" 부트에서는 보안상 토큰·온보딩 플래그를 폐기한다. 이때 "전부 건너뛴" 사용자(서버 프로필 전무)는 빈 온보딩이 1회 재노출될 수 있음(데이터 손실 없음, 재건너뛰기로 해소). 해결하려면 서버 온보딩 표식이 필요하나 rich1 무수정 원칙(§0-1)과 충돌 — 보안 우선으로 수용.
